...
Переименуйте `.env.example` в `.env` и задайте значения переменных окружения.

Для запуска auth сервиса, PostgreSQL, Redis и Nginx используйте команду:

`docker compose up --build`

Документация OpenAPI доступна на странице [http://127.0.0.1/api/openapi](http://127.0.0.1/api/openapi)

Для добавления суперпользователя из консоли необходимо воспользоваться командой:

`docker exec -it -e PYTHONPATH=. auth_sprint_1-auth-1 python cli/admin_create.py`
